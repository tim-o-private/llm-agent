...
15. **Dual-import singleton bug** — When `chatServer/main.py` runs directly, Python's sys.path includes both `chatServer/` and the project root. `except ImportError` fallbacks MUST use bare module paths (`from database.supabase_client import ...`), never `from chatServer.database.supabase_client import ...`. Mixing styles creates two module instances with separate globals, breaking singletons silently. The `database.connection` module masks this via auto-init; `supabase_client` now has the same fallback.
...
18. **Zustand `getState()` is not reactive** — `useAuthStore.getState().user` returns a snapshot that does NOT trigger re-renders. Inside React hooks/effects, always use `useAuthStore((s) => s.user)` selector subscriptions. `getState()` is only safe in event handlers, callbacks, and non-React code. See gotcha #3 (same root cause for auth tokens).
19. **Cross-channel message polling** — `useChatStore` loads messages once on init via `loadHistoricalMessages`. For cross-channel sync (e.g., Telegram messages appearing in web), the store needs explicit polling (`refreshMessages` action + `setInterval`). There is no WebSocket/SSE push.
20. **assistant-ui Thread auto-scroll** — `ThreadPrimitive.Viewport` only auto-scrolls when `isAtBottom` is already true. On initial message hydration from an external store, the viewport starts at `scrollTop=0` so auto-scroll never fires. Must manually scroll with retry delays (100/300/600ms) since the Thread renders asynchronously.
21. **Always commit new files before pushing** — When creating new service files, test files, or modules, `git add` them explicitly. Untracked files won't appear in `git diff` and are easy to miss, causing CI import failures.
