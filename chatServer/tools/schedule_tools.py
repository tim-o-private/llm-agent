...
class CreateScheduleTool(BaseTool):
    ...
    def prompt_section(cls, channel: str) -> str | None:
        ...
        if channel in ("web", "telegram"):
            return "Schedules: When the user wants recurring work (daily summaries, weekly reports), use create_schedule with a cron expression. Use list_schedules to show existing schedules."
        else:
            return None
