...
# Local cache for active agent executors within this router: (user_id, agent_name) -> AgentExecutor
...
async def send_message_to_agent(payload: ChatMessageIn):
    ...
    # Use a default agent_name for now, e.g., "assistant" or "coach"
    # This should ideally come from the frontend or be configurable
    agent_name_to_use = os.getenv("DEFAULT_CHAT_agent_name", "assistant") 
    user_id_to_use = payload.userId or "default_user" # Fallback if userId is not provided

    agent_key = (user_id_to_use, agent_name_to_use)
    ...
    try:
        ...
        print(f"[ChatRouter] Agent configuration error for {agent_name_to_use} (User: {user_id_to_use}): {e}")
        # Sanitize error for client
        ai_reply = f"Error: Agent configuration not found for '{agent_name_to_use}'. Please contact support."
        # Consider raising HTTPException for specific client feedback if appropriate
        # raise HTTPException(status_code=500, detail=f"Agent configuration error for {agent_name_to_use}. {e}")
    except ImportError as e:
        print(f"[ChatRouter] Failed to import agent dependencies for {agent_name_to_use} (User: {user_id_to_use}): {e}")
        ...
        print(f"[ChatRouter] Error processing message with agent {agent_name_to_use} for user {user_id_to_use}: {e}")
