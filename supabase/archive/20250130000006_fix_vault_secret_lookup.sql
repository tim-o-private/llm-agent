-- Option A: Fix vault secret lookup to check vault.secrets directly by name
-- This handles orphaned vault secrets properly after revoke operations
...
DECLARE
  ...
  existing_access_secret RECORD;
  existing_refresh_secret RECORD;
BEGIN
  ...
  SELECT id INTO existing_access_secret
  FROM vault.secrets 
  WHERE name = access_secret_name;

  SELECT id INTO existing_refresh_secret
  FROM vault.secrets 
  WHERE name = refresh_secret_name;

  -- Handle access token
  IF existing_access_secret.id IS NOT NULL THEN
    -- Update existing vault secret
    SELECT vault.update_secret(
      existing_access_secret.id,
      p_access_token
    ) INTO access_secret_id;
    
    -- Ensure we got the same ID back
    IF access_secret_id IS NULL THEN
      access_secret_id := existing_access_secret.id;
    END IF;
  ELSE
  ...
  IF p_refresh_token IS NOT NULL THEN
    IF existing_refresh_secret.id IS NOT NULL THEN
      -- Update existing vault secret
      SELECT vault.update_secret(
        existing_refresh_secret.id,
        p_refresh_token
      ) INTO refresh_secret_id;
      
      -- Ensure we got the same ID back
      IF refresh_secret_id IS NULL THEN
        refresh_secret_id := existing_refresh_secret.id;
      END IF;
    ELSE
      -- Create new vault secret
      SELECT vault.create_secret(
