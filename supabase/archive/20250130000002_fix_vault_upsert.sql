-- Fix vault secret upsert to handle existing secrets
-- Update the RPC function to upsert vault secrets instead of always creating new ones
...
BEGIN
  ...
  IF access_secret_id IS NOT NULL THEN
    ...
    SET secret = p_access_token, updated_at = NOW()
    WHERE id = access_secret_id;
  ELSE
    -- Create new secret
  ...
  -- Handle refresh token if provided (upsert)
  IF p_refresh_token IS NOT NULL THEN
    ...
    IF refresh_secret_id IS NOT NULL THEN
      -- Update existing secret
      UPDATE vault.secrets 
      SET secret = p_refresh_token, updated_at = NOW()
      WHERE id = refresh_secret_id;
    ELSE
      -- Create new secret
      SELECT vault.create_secret(
        p_refresh_token,
