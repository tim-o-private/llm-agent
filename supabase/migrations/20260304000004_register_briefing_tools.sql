-- SPEC-028: Register manage_briefing_preferences tool
-- type='system', approval_tier='auto' — no user approval needed

-- 1. Register manage_briefing_preferences in tools table (idempotent)
...
-- 2. Link manage_briefing_preferences to assistant agent
...
SELECT
    ...
    true
FROM tools t
WHERE t.name = 'manage_briefing_preferences'
AND NOT EXISTS (
