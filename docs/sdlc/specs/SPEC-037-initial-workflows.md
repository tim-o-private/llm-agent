...
|--------------|-------------|
| SPEC-028 (Morning & Evening Briefings) | **Deprecated by this spec.** SPEC-028 implemented briefings as single-shot `ScheduledExecutionService` calls with a synthesis prompt. This spec replaces that execution path with multi-step workflows. SPEC-028's `BriefingService` (preferences CRUD, deferred observations), `user_preferences` table, and `deferred_observations` table are reused. The old `generate_morning_briefing()` / `generate_evening_briefing()` methods and their direct `ScheduledExecutionService` calls are removed — job handlers dispatch workflows instead. |
...
- [ ] **AC-13:** The morning briefing workflow replaces SPEC-028's execution path. The existing `handle_morning_briefing` job handler in `job_handlers.py` is updated to dispatch the workflow via `WorkflowRunManager.start_run("morning-briefing", params)` instead of directly calling `BriefingService.generate_morning_briefing()`. The old `generate_morning_briefing()` method is removed from `BriefingService`. The self-scheduling pattern (next occurrence) is preserved. [A11]
...
- [ ] **AC-21:** The approval response supports three actions: `approve` (proceed to send), `reject` (cancel the workflow), and `revise` (restart from compose-draft with new instructions). The `revise` action includes user feedback text that is injected into the compose-draft step's parameters on re-execution. Maximum 3 revisions per workflow run — after the third revision, the gate returns: "I'm struggling with this one — want to write it yourself?" and the workflow is cancelled. Revision count is tracked in `WorkflowState`. [A12]
...
| `chatServer/services/briefing_service.py` | Remove `generate_morning_briefing()` and `generate_evening_briefing()` methods (replaced by workflows). Retain: `get_user_preferences()`, `update_user_preferences()`, `format_for_telegram()`, deferred observation CRUD |
...
- **Revision with contradictory instructions:** Each revision appends to the instruction list. The LLM resolves conflicts by prioritizing the most recent instruction (last one wins). No special handling needed — this is natural LLM behavior. Maximum 3 revisions per run (AC-21) — after that, the workflow cancels with a friendly message suggesting the user write the reply themselves.
- **Workflow step API error:** The engine handles per-step errors. If a step fails (e.g., API rate limit), the run status is set to `failed` with the error. The job handler's retry mechanism resumes from the failed step's checkpoint (per SPEC-036 resolved decision #2), preserving completed step outputs.
...
1. **Email triage disabled by default.** `email_triage_enabled` defaults to `false`. No scheduling without a data source — users must have Gmail connected and explicitly opt in. This avoids scheduling jobs that will immediately fail for users without email.

2. **Revision cap: 3 maximum.** After 3 revisions, the gate returns "I'm struggling with this one — want to write it yourself?" and the workflow is cancelled. Prevents runaway token costs from infinite revision loops.

3. **SPEC-028 deprecated — old path removed.** The morning-briefing workflow replaces SPEC-028's `generate_morning_briefing()` / `generate_evening_briefing()` direct execution path. Job handlers are updated to dispatch workflows instead. The old `BriefingService.generate_*_briefing()` methods are removed in this spec (FU-5). `BriefingService` retains: `get_user_preferences()`, `update_user_preferences()`, `format_for_telegram()`, and deferred observation management.

4. **Haiku for classification, Sonnet for generation — confirmed.** Email triage `categorize` step uses `claude-haiku-4-5-20251001` (structured classification task, cost-effective). All generation/composition steps use default Sonnet. Per-step model override is a SPEC-036 capability (AC-08).
