...
async function fetchAiResponse(message: string): Promise<string> {
  ...
  try {
    const response = await fetch('/api/chat', { // Vite proxy will handle this
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        user_id: 'test_user_123', // Hardcoded for now, replace with actual user ID from auth
        agent_id: 'assistant', // Hardcoded for now, could be dynamic
        message: message,
      }),
    });
...
export const ChatPanel: React.FC = () => {
  ...
  const handleSendMessage = async (messageText: string) => {
    // const messageText = inputValue.trim(); // inputValue is now passed as arg
