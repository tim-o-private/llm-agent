...
const TaskForm = React.forwardRef<TaskFormRef, TaskFormProps>(({
  ...
  const transformToForm = React.useCallback((task: Task | undefined): TaskFormData => ({
    title: task?.title || '',
  ...
  const saveEntity = React.useCallback(async (
    ...
    if (isCreating) {
      ...
      const userIdToSet = parentTaskId && useTaskStore.getState().getTaskById(parentTaskId)?.user_id || currentUserId;
  ...
  const {
    ...
    saveError,
    isSaving,
  } = useRadixForm<Task, TaskFormData>({
    entityId: taskId,
      ...
      >
        <Form.Field name="title">
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
            <Form.Label className="block text-sm font-medium text-text-secondary">
              Title
            </Form.Label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Form.Field name="status">
            <Form.Label className="block text-sm font-medium text-text-secondary mb-1">
              Status
            </Form.Label>
            <Form.Control asChild>
              <Select
