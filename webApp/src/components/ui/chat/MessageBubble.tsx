...
export const MessageBubble: React.FC<MessageBubbleProps> = ({ text, sender, timestamp, avatarUrl, senderName }) => {
  ...
  const alignmentClasses = clsx('flex mb-3', {
    'justify-end': isUser,
  ...
  const avatar = (
    <div className={clsx('w-8 h-8 rounded-full bg-neutral-subtle flex-shrink-0', isUser ? 'ml-2' : 'mr-2')}>
      {avatarUrl && (
        <img
          src={avatarUrl}
          alt={`${senderName || sender}'s avatar`}
          className="w-full h-full rounded-full object-cover"
        />
      )}
      {!avatarUrl && senderName && (
        <span className="flex items-center justify-center w-full h-full text-xs text-neutral-strong font-semibold">
          {senderName.substring(0, 2).toUpperCase()}
        </span>
      )}
      {!avatarUrl && !senderName && sender === 'ai' && (
        <span className="flex items-center justify-center w-full h-full text-xs">🤖</span>
      )}
      {!avatarUrl && !senderName && sender === 'user' && (
        <span className="flex items-center justify-center w-full h-full text-xs">👤</span>
      )}
    </div>
  );
