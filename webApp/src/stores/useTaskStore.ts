    ...
    immer((set, get) => ({
      ...
      updateTask: (id, updates) => {
        ...
        set((state) => {
          ...
          console.log(
            `[useTaskStore] Task ID: ${id} - Before:`,
            oldTask,
            'After:',
            JSON.parse(JSON.stringify(state.tasks[id])),
          );
      ...
      getSubtasksByParentId: (parentId) => {
        return Object.values(get().tasks)
          .filter((task) => task.parent_task_id === parentId && !task.deleted)
          .sort((a, b) => {
      ...
      getTopLevelTasks: () => {
        return Object.values(get().tasks)
          .filter((task) => !task.parent_task_id && !task.deleted)
          .sort((a, b) => {
