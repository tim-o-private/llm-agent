...
export const useTaskStoreInitializer = () => {
  const { tasks, isLoading, error, initialized, initializeStore } = useTaskStore((state) => ({
    tasks: state.tasks,
  ...
  useEffect(() => {
    ...
    if (!initialized && !isLoading) {
      ...
      console.log(
        `[useTaskStoreInitializer] Store ${initialized ? 'already initialized' : 'is loading'}, skipping initialization`,
      );
    }
