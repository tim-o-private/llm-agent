...
export function useSubtaskManagement(taskId: string | null): SubtaskManagementHook {
  ...
  const createSubtask = useCallback((subtaskData: Partial<Task>, tempId: string): Task => {
  ...
  const handleSubtaskReorder = useCallback((reorderedSubtasks: Task[]) => {
    optimisticState.handleReorder(reorderedSubtasks);
  }, [optimisticState.handleReorder]);

  const handleSubtaskCreate = useCallback((subtaskData: Partial<Task>) => {
  ...
  const handleSubtaskUpdate = useCallback((id: string, updates: Partial<Task>) => {
  ...
  const applyPendingSubtaskChanges = useCallback(async (currentTaskId: string) => {
    ...
    if (pendingChanges.reorders.length > 0) {
      ...
      const positionUpdates = latestReorder.items.map(async (subtask, index) => {
